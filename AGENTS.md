...
File Map ≤12 rows · SESSION_PRIMER ≤150 lines · this file ≤65 lines (paid every message, kept
tight — Learned/Fixed Rules live in `wiki/PROJECT_BACKGROUND.md` instead, unlike soulmate-3's
never-auto-loaded AGENTS.md). No live token-usage signal exists mid-session: `design` sizes
sub-tasks to finish well under your model's context length by construction; `build` checkpoints
the moment actual work overruns that plan.
