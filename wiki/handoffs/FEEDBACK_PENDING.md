...
| 5 | `COMMITS_WITHOUT_PRIMER_THRESHOLD = 4` (the new elective-arm fix, L07) is an untuned starting guess — not validated against a real, larger multi-file sub-task to check it doesn't false-positive-block legitimate work | p2 | open | Documented as tunable in the plugin's own comment; needs a round 3 test with a sub-task that legitimately produces 3-4 file commits before its primer update | round 2 |
