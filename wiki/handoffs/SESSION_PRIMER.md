# SESSION PRIMER — round 23 re-scored (turnkey 82, structural 81 — pipeline reorder held, backtick-guarantee wording found imprecise); round 24 wording fix done, not yet re-scored
...
| 23 (re-score) | Independently reproduced round 23's fix (own fresh files, not copied) — backtick-quoted token no longer false-positives; round 18's wrap-split-code-span case unregressed | **turnkey 82 (held), structural 79→81**. Own adversarial angle: a wrap-split backtick span (opens one line, closes next) containing `<!--` still silently swallows real content later — pre-existing, not a round-23 regression, but the "backtick-quoting is the real mitigation" wording doesn't say it's same-line-only | fixed round 24 (wording only, no logic change) |
